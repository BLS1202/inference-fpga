    ...
    always_ff @(posedge clk or negedge rst_l) begin
        if (!rst_l) begin
            ...
            case (state)
                ...
                // Stage 1: calculate the LUT address and register it.
                ST_EXP_ADDR: begin
                ...
                // Stage 2: register the LUT output before accumulation.
                ST_EXP_READ: begin
                    exp_value_reg <= exp_lut[exp_lut_addr_reg];
                    state <= ST_EXP_ACC;
                end

                // Stage 3: accumulate the registered exponential value.
                ST_EXP_ACC: begin
                    if (index <= valid_position) begin
                        exp_arr[index] <= exp_value_reg;
                        exp_sum <= exp_sum + exp_value_reg;
                    end else begin
                        exp_arr[index] <= '0;
                    end
