...
// Nexys A7 wrapper for the original rtl/top_inference implementation.
// sw[0] starts one inference request and sw[1] is an active-high reset.
module nexys_a7_inference_top #(
    ...
    always_ff @(posedge CLK100MHZ or negedge rst_l) begin
        if (!rst_l) begin
            ...
            token_id <= next_token;
            if (pos_id == POS_WIDTH'(BLOCK_SIZE - 1)) begin
                pos_id <= '0;
            end else begin
                pos_id <= pos_id + 1'b1;
            end
